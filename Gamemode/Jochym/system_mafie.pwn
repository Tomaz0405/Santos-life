...
CMD:zwiaz(playerid,cmdtext[])
{
	...
	if(!GraczaMaTypPrzedmiotu(playerid, P_SZNOR))
	{
	    dShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "• TIP:", "Aby u¿yæ tej komendy, musisz posiadac w ekwipunku sznur!", "Zamknij", "");
		return 1;
	}
	...
	{
		dShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "• TIP:", "Aby zwi¹zaæ gracza wpisz: /zwiaz [id gracza]", "Zamknij", "");
		return 1;
	}
	if(!PlayerObokPlayera(playerid, playerid2, 3))
	{
		dShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "• TIP:", "Gracz którego chcesz zwi¹zaæ jest zbyt daleko od ciebie.", "Zamknij", "");
		return 1;
	}
	...
	{
		dShowPlayerDialog(playerid, DIALOG_INFO, DIALOG_STYLE_MSGBOX, "• TIP:", "Gracz którego próbujesz zwi¹zaæ jest skuty!", "Zamknij", "");
		return 0;
	}
	if(DaneGracza[playerid2][gSznur] == -1)
	{
		new str[250];
	    DaneGracza[playerid2][gSznur] = playerid;
		DaneGracza[playerid][gKajdankiS] = 1;
		if(IsPlayerInAnyVehicle(playerid2))
		{
			Frezuj(playerid2, 0);
			format(str, sizeof(str), "przywi¹zuje %s do pojazdu.", ZmianaNicku(playerid2));
		}else{
			format(str, sizeof(str), "za³o¿y³ sznur %s na rêce.", ZmianaNicku(playerid2));
		}
	...
	{
		new str[250];
	    DaneGracza[playerid2][gSznur] = -1;
		DaneGracza[playerid][gKajdankiS] = 0;
	    Frezuj(playerid2, 1);
	  	format(str, sizeof(str), "zdj¹³ sznur z r¹k %s.", ZmianaNicku(playerid2));
	    cmd_fasdasfdfive(playerid, str);
	}
	return 1;
}
...
stock GraczNaTerenieV(playerid)
{
	new Float:x, Float:y, Float:z;
	GetPlayerPos(playerid, x, y, z);
    ForeachEx(zoneid, MAX_ZONE)
	{
